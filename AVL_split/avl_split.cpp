...
void checkSplit(struct node *root, int left, int k){
    if(root==NULL) return;
    if(left) {
        if(root->value > k) {
            for(int i = 0; i < 100 ; ++i) {
            ...
            checkSplit(root->right, left, k);
            checkSplit(root->left, left, k);
        }
    }
    else{
        if(root->value < k) {
...
struct node *split (struct node *root, int k) {
    if(root == NULL) {
        struct node *x = new_node(0);
        x->left = x->right = NULL;
        // x->height = 1;
        return x;
    }
    if(root->value > k){
        struct node *x = split(root->left, k);
        struct node *y = merge(x->right, root->right);
        y = insert(y, root->value);
        struct node *z = new_node(0);
        z->left = x->left;
        z->right = y;
        ...
        struct node *x = split(root->right, k);
        struct node *y = merge(x->left, root->left);
        y = insert(y, root->value);
        struct node *z = new_node(0);
        z->left = y;
        z->right = x->right;
